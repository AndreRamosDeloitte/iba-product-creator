  ...
  useEffect(() => {
    async function fetchCarriers() {
      ...
      try {
        ...
        const partiesUrl = `/api/parties?partyTypeName=Risk Carrier`
        const partiesResp = await fetch(partiesUrl, {
          headers: {
        ...
        if (!partiesResp.ok) throw new Error('parties request failed')
        const partiesJson = await partiesResp.json()
        const getArray = (v: unknown): Array<Record<string, unknown>> | null => {
          if (Array.isArray(v)) return v as Array<Record<string, unknown>>
          if (v && typeof v === 'object') {
            const o = v as Record<string, unknown>
            if (Array.isArray(o['elements'])) return o['elements'] as Array<Record<string, unknown>>
          }
          return null
        }
        const items = getArray(partiesJson)
        if (!items) throw new Error('unexpected parties response')
